...
Confirm saved status, public URLs, selected build, current Xcode/SDK eligibility, metadata/build consistency, commerce dependency set, review access, privacy agreement, conditional-account/login/UGC/regulated requirements, screenshot processing, release control, and support availability. Invoke `$ios-release-tester` and `$app-compliance-auditor` before submitting.
