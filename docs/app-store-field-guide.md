...
Read [`current-apple-requirements.md`](current-apple-requirements.md) before using this field guide. It contains the dated 2026 upload, metadata, privacy, accessibility, account, and distribution baseline.
...
- Accessibility Nutrition Labels only for features proven across all common tasks on each declared device; currently voluntary but moving toward a future submission requirement.
- App tags: review US tags inferred from metadata and remove inaccurate selections.
- Regulated medical device status when category/content triggers the declaration.
...
- The build was created with the currently required Xcode and platform SDK.
- Account creation, third-party login, UGC, uploads, health/regulated features, encryption, and territory triggers were evaluated.
